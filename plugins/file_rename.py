...
async def rename_start(client, message):
    ...
    try:
        await message.reply_text(
            text=
            f"**__Pʟᴇᴀꜱᴇ Eɴᴛᴇʀ Nᴇᴡ Fɪʟᴇɴᴀᴍᴇ...__**\n\n**Oʟᴅ Fɪʟᴇ Nᴀᴍᴇ** :- `{filename}`",
...
async def refunc(client, message):
    ...
    if (reply_message.reply_markup) and isinstance(reply_message.reply_markup,
                                                   ForceReply):
        ...
        if file.media in [MessageMediaType.VIDEO, MessageMediaType.DOCUMENT]:
            button.append([
                InlineKeyboardButton("🎥 Vɪᴅᴇᴏ", callback_data="upload_video")
            ])
        elif file.media == MessageMediaType.AUDIO:
            button.append([
                InlineKeyboardButton("🎵 Aᴜᴅɪᴏ", callback_data="upload_audio")
            ])
        await message.reply(
            text=
...
async def doc(bot, update):
    ...
    try:
        path = await bot.download_media(message=file,
                                        ...
                                        progress_args=("Dᴏᴡɴʟᴏᴀᴅ Sᴛᴀʀᴛᴇᴅ....",
                                                       ms, time.time()))
    ...
    try:
        if type == "document":
            await bot.send_document(update.message.chat.id,
                                    ...
                                    caption=caption,
                                    progress=progress_for_pyrogram,
                                    progress_args=("Uᴩʟᴏᴅ Sᴛᴀʀᴛᴇᴅ....", ms,
                                                   time.time()))
        elif type == "video":
